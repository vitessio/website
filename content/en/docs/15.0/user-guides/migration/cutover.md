...
- Check the information in it to see what might need to be adjusted
...
Dry Run results for SwitchTraffic run at 02 Jan 06 15:04 MST
...
```
$ vtctlclient --server localhost:15999 SwitchTraffic -- --dry_run targetkeyspace.workflowname
```
...
Routing rules for tables [t1] will be updated
SwitchTraffic completed, freeze and delete vreplication streams on:
        tablet 900
...
Any subsequent SwitchTraffic will fail.
...
If the VReplication of changes from the sourcekeyspace to the targetkeyspace are lagging (possibly because of high write rate to the source keyspace), the SwitchTraffic for writes may fail.
This is because as part of switching traffic for writes, traffic is paused, and Vitess then waits a short amount of time for the targetkeyspace shards to catch up to the point(s) where the sourcekeyspace shard(s) were stopped.
If this does not happen within that timeout period, the SwitchTraffic will fail.
The default for this wait period is 30 seconds; and can be adjusted upwards or downwards by passing the --timeout flag to the SwitchTraffic command.

{{< info >}}
Note that the above limitation does not apply to SwitchTraffic for reads, since replica/rdonly instances are assumed to lag anyway.  You should accordingly manually validate that the replica lag is within acceptable limits for your purposes before you switch read traffic.   You can do this by inspecting the MaxVReplicationLag value in the Workflow … show output (cf. above).  This value represents the maximum lag, in seconds, of the underlying streams.
{{< /info >}}

## Cutover rollback

MoveTables supports cutover rollbacks via the MoveTables --ReverseTraffic command.
...
If the reverse workflow cannot keep the sourcekeyspace up to date with the targetkeyspace, which is now receiving the application writes, the reverse SwitchTraffic for writes may time out.
If that occurs, you may need to temporarily stop the application writes to allow the sourcekeyspace to catch up to the target keyspace. You would then retry the SwitchTraffic and then resume the application writes. This should not be an issue if the amount of writes to the keyspaces are similar before and after the cutover.
