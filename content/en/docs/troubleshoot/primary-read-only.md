...
However, to fix manually, you can use the [`SetReadWrite`](../../reference/command-line-reference/vtctl/tablets/#setreadwrite) command to do that. Usually if something unexpected happens to the primary, it's better to reparent to a different replica with [`EmergencyReparentShard`](../../reference/command-line-reference/vtctl/shards/#emergencyreparentshard). 

If you need to do planned maintenance on the primary, it's best to first reparent to another replica with [`PlannedReparentShard`](../../reference/command-line-reference/vtctl/shards/#plannedreparentshard).
