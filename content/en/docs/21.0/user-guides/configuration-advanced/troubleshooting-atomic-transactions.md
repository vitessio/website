...
1. Get more information about the transaction by querying the `redo_state` and `redo_statement` tables in all the participating shards using the dtid.
2. For each shard, it will have the status of the transaction on that shard. For the shards where the transaction has failed, it will have the writes that were part of the transaction.
