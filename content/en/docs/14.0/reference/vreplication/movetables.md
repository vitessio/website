...
Specifying this flag causes any DATETIME fields to be converted from given time zone into UTC. It is expected that the application has
stored *all* DATETIME fields, in all tables being moved, in the specified time zone. On the target these datetimes will be stored in UTC.
...
It is expected that the time zone tables are loaded on the target mysql servers (for example, by executing the script generated by `mysql_tzinfo_to_sql /usr/share/zoneinfo`). 
...
perform any conversions using, for example, connection settings or `convert_tz()`.
