...
| -queryserver-config-schema-change-signal |  | query server schema signal, will signal connected vtgates that schema has changed whenever this is detected. VTGates will need to have -schema_change_signal enabled for this to work |
...
| -queryserver-config-terse-errors |  | prevent bind vars from escaping in returned or logged errors |
