...
Vitess has a limited support for temporary tables. It works only for unsharded keyspace.

If the user create a temporary table then the session will start using reserved connections for any query sent on that session 
and also the plans generated by those will not be caches. But, they will continue to use the plan caches from other sessions.  
