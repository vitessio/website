...
### app_conn_pool
  ...
  * vttablet user flag:  `-db_app_user` default 'vt_app')
  	* Used for VReplication in v7.0 and onwards to setup filtered binlog stream from a source shard MySQL instance. Also used for actual copying of the upstream source table rows in VReplication, if necessary. Note that before v7.0 db_dba_user was used for VReplication.
