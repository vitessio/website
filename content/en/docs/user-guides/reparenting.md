...
4. Ensures that the new master's state is updated correctly and that the new master is not a MySQL replica of another server. It runs the MySQL `SHOW SLAVE STATUS` command, ultimately aiming to confirm that the MySQL `RESET SLAVE` command already executed on the tablet.
5. Updates, for each replica, the topology service record and replication graph to reflect the new master. If the old master does not return successfully in this step, Vitess changes its tablet type to spare to ensure that it does not interfere with ongoing operations.
